...
class NewDeviceWithWebhookScript(Script):

    class Meta:
        name = "Create Device with Webhook"
        description = "Create a single device in an existing site with local context data from a POP device and send a webhook notification"
